...
namespace WPFClinica.Scripts.DB {
    ...
    public class Historico_ProcedimentosLab {
        ...
        public Historico_ProcedimentosLab(MySqlH handler) {
            Handler = handler;
        }
        ...
        /// <param name="for_each">CPF, RealizadoEm, Pago, ProcedimentoLab, Convenio, NomePaciente, FormaPagamento</param>
        public void SelectLike(string procedimento, string convenio, DateTime inicio, DateTime fim, string paciente, bool includeAll, Action<string, DateTime?, double, string, string, string, List<(PaymentMethods, double)>> for_each) {
            string c =
                $"select " +
                $"  a.{nameof(ID)}," + //0
                ...
                $"  d.Nome " + //9
                $"from {Name} as a " +
                $"left join ProcedimentosLab as b on a.Procedimento = b.ID " +
                $"left join Convenios as c on a.Convenio = c.ID " +
                $"left join Pessoas as d on a.Paciente = d.CPF " +
                $"where " +
                $"  b.Procedimento like concat(@proc, '%') and " +
                $"  c.Convenio like concat(@con, '%') and " +
                $"  (a.PagoEm between @ini and @fim) and " +
                $"  d.Nome like concat('%' , @nome_paciente, '%') " +
                $"  {(includeAll ? "" : $"and a.PagoEm is not null")} and " +
                $"  Is_enabled = 1 " +
                $"order by a.PagoEm desc " +
                $"limit 10000;";
            Handler.QueryRLoop(c, (r) => {
                var ID = r.GetInt32(0);
                var Procedimento_ID = r.GetInt32(1);
                var Con_ID = r.GetInt32(2);
                var CPF = r.GetString(3);
                var RealizadoEm = r.GetNullDateTime(4);
                var PagoEm = r.GetNullDateTime(5);
                var Pago = r.GetDouble(6);
                var ProcedimentoLab = r.GetString(7);
                var Convenio = r.GetString(8);
                var nome_paciente = r.GetString(9);
                var formaPagamento = new List<(PaymentMethods, double)>();
                Handler.QueryRLoop($"select Pagamento, Valor from procedimentoslab_formapagamento where Historico = {ID};", (re) => {
                    formaPagamento.Add((re.GetInt32(0) switch {
                ...
                for_each(CPF, RealizadoEm.HasValue? RealizadoEm : PagoEm, Pago, ProcedimentoLab, Convenio, nome_paciente, formaPagamento);
            },
                ("@proc", procedimento),
                ("@con", convenio),
                ("@ini", inicio.ToString("yyyy-MM-dd HH:mm:ss")),
                ("@fim", fim.ToString("yyyy-MM-dd HH:mm:ss")),
